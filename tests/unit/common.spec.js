...
describe('Unit test for Acl functionality', () => {
  context('Test Deny function', () => {
    ...
    it('Should return default deny Object when called without arguments', done => {
    ...
    it('Should return Object with custom message when custom message is defind', done => {
  ...
  context('Test assertIsGlobOrArray', () => {
    ...
    it('Should throw an error if the methods supplied is string but not *', done => {
      try {
        ...
        assert.equal(message, 'DefinitionError: Unrecognised glob "?", use "*" instead.');
      }
      done();
    });
  });
  ...
  context('Test readConfigFile function', () => {
    ...
    it('Should read the correct file', done => {
      ...
      // Read yaml file
      const expectedConfig = yaml.safeLoad(fs.readFileSync('tests/config/nacl.yaml'));
      const yamlConfig = readConfigFile('tests/config/nacl.yaml');
      assert(yamlConfig, true);
      assert(Array.isArray(yamlConfig), true);
      assert.deepEqual(yamlConfig, expectedConfig);
      done();
    });
    ...
    it('Should throw an error is file is missing', done => {
      try {
        ...
        assert.equal(message, 'TypeError: Path must be a string. Received undefined.');
      }
      done();
    });
  });
  ...
  context('Test mapPolicyToGroup function', () => {
    ...
    it('Should only map valid policy', done => {
      ...
      try {
        ...
        assert.equal(message, 'TypeError: action should be either "deny" or "allow".');
      }
      done();
    });
  ...
  context('Test findRoleFromRequest', () => {
    ...
    it('Should find role from deocded and session', done => {
    ...
    it('Should return default role if none is defined', done => {
      const req = http.createRequest();
      let role = findRoleFromRequest({ request: req }, null, 'guest');
      assert(role, true);
      assert.equal(role, 'guest');
      done();
    });

    it('Should return role from sepecified decoded object name', done => {
      const req = http.createRequest();
      req.user = {
        credentials: {
          role: 'user'
      ...
      let role = findRoleFromRequest({ request: req }, null, null, 'user.credentials');
      assert(role, true);
      assert(role, 'user');
      done();
    });
    ...
    it('Should return role from specified search path', done => {
      ...
      let role = findRoleFromRequest({ request: req }, 'user.credentials.role');
      assert(role, true);
  ...
  context('Test findPermissionForRoute function', () => {
    ...
    it('Should return apporpriate permissions given a specific route', done => {
      const policy = [
        {
          resource: 'users/*',
          methods: ['POST', 'GET', 'PUT'],
      ...
      let permissions = findPermissionForRoute('/users/public', 'GET', '', policy);
      ...
      permissions= findPermissionForRoute('/api/users/king', 'GET', 'api', policy);
  ...
  context('Test matchUrlToResponse', () => {
    it('Should exist and should be a function', done => {
    ...
    it('Should return true or false when given url and response', done => {
  ...
  context('Test checkIfHasAccess function', () => {
    it('Should exist and should be a function', done => {
    ...
    it('Should return appropriate response when checking for access', done => {
      ...
      assert.deepEqual(checkIfHasAccess('GET', ctx, next, permission), next());
      checkIfHasAccess('DELETE', ctx, next, permission);
  ...
  context('Test isAllowed function', () => {
    it('Shoule exist and should be a function', done => {
